...
import { Activity, Sparkles, Calendar, Clock, TrendingUp, ArrowRight } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useHabitStore } from '../stores/habitStore';
import { formatDateToYYYYMMDD, formatDateForDisplay, isToday } from '../utils/date';
...
const Dashboard = () => {
  ...
  const { getActiveHabits, isHabitDueToday, getCompletionRateForDate } = useHabitStore();
      ...
      <div className="card overflow-hidden">
        ...
        {habitsForToday.length === 0 ? (
          <div className="mt-6 rounded-lg bg-gray-50 p-4 text-center dark:bg-gray-700/50">
            ...
            >
              <Activity size={32} className="mx-auto mb-2 text-gray-400 dark:text-gray-500" />
          ...
          {activeHabits.length === 0 ? (
            ...
            <div className="space-y-3">
              {activeHabits.slice(0, 3).map((habit) => {
                const streak = useHabitStore((state) => state.getHabitStreak(habit.id).current);
                return (
                  <div key={habit.id} className="flex items-center justify-between rounded-md bg-gray-50 p-3 dark:bg-gray-700/50">
                    ...
                    <div className="flex items-center">
                      <Sparkles 
                        size={14} 
                        className={`mr-1 ${streak > 0 ? 'text-yellow-500' : 'text-gray-400'}`} 
                      />
                      <span className={`text-sm font-medium ${streak > 0 ? 'text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400'}`}>
        ...
        <div className="card">
          ...
          <div className="space-y-4">
            {activeHabits.length === 0 ? (
              ...
              <>
                {activeHabits.slice(0, 3).map((habit) => {
                  const completionRate = useHabitStore((state) => 
                    state.calculateCompletionRate(habit.id, 7)
                  );
